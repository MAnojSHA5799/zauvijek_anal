...
> = {
  "Pattern Making": {
    fishbone: [
      { label: "Man", causes: ["Untrained workers", "Manual errors", "Lack of skill development", "Poor supervision"] },
      { label: "Machine", causes: ["Old CNC tools", "Poor calibration", "Equipment downtime", "Maintenance issues"] },
      { label: "Method", causes: ["Lack of SOPs", "Improper dimensioning", "No standardization", "Poor workflow"] },
      {
        label: "Material",
        causes: ["Substandard wax", "Low-quality sand", "Material inconsistency", "Storage issues"],
      },
      {
        label: "Measurement",
        causes: ["Uncalibrated gauges", "Improper readings", "Tolerance issues", "No verification"],
      },
      {
        label: "Environment",
        causes: ["Humidity issues", "Dust in workspace", "Temperature variation", "Poor lighting"],
      },
    ],
    roi: {
  ...
  "Metal Melting": {
    fishbone: [
      { label: "Man", causes: ["Lack of training", "Operator errors", "Safety violations", "Inexperience"] },
      { label: "Machine", causes: ["Crucible damage", "Sensor errors", "Furnace inefficiency", "Power fluctuations"] },
      {
        label: "Method",
        causes: ["Incorrect melt timing", "Poor temperature control", "No optimization", "Wrong sequence"],
      },
      { label: "Material", causes: ["Low quality alloy", "Contamination", "Composition variation", "Impurities"] },
      {
        label: "Measurement",
        causes: ["Incorrect temp readings", "Calibration issues", "Sensor drift", "No monitoring"],
      },
      { label: "Environment", causes: ["Heat loss", "Ambient conditions", "Ventilation problems", "Humidity"] },
    ],
    roi: {
  ...
  Cooling: {
    fishbone: [
      {
        label: "Man",
        causes: ["No cooling time observation", "Improper handling", "Lack of monitoring", "Poor timing"],
      },
      { label: "Machine", causes: ["Ineffective chillers", "Poor heat exchange", "Equipment failure", "Pump issues"] },
      {
        label: "Method",
        causes: ["Overcooling or undercooling", "No control strategy", "Poor timing", "Wrong parameters"],
      },
      {
        label: "Material",
        causes: ["Wrong alloy cooling rate", "Thermal properties", "Composition effects", "Size variation"],
      },
      { label: "Measurement", causes: ["No time logs", "Temperature errors", "Poor tracking", "Sensor failure"] },
      {
        label: "Environment",
        causes: ["Ambient temperature variation", "Air flow issues", "Heat sources", "Seasonal changes"],
      },
    ],
    roi: {
  ...
  "Fettling & Finishing": {
    fishbone: [
      {
        label: "Man",
        causes: ["Missed burrs and sharp edges", "Inconsistent technique", "Quality oversight", "Fatigue"],
      },
      { label: "Machine", causes: ["Worn out grinders", "Tool degradation", "Poor maintenance", "Vibration"] },
      { label: "Method", causes: ["Improper trimming pattern", "No standardization", "Poor sequence", "Wrong tools"] },
      { label: "Material", causes: ["Excess metal flash", "Hard spots", "Material variation", "Surface defects"] },
      {
        label: "Measurement",
        causes: ["No surface roughness check", "Dimension errors", "Poor inspection", "Missing tools"],
      },
      { label: "Environment", causes: ["Inadequate lighting", "Dust", "Vibration", "Noise levels"] },
    ],
    roi: {
  ...
  return (
    <div className="relative bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-12 rounded-3xl shadow-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
      ...
      <div className="text-center mb-12 relative z-10">
        ...
        <p className="text-xl text-slate-600 dark:text-slate-400 font-medium">{title} Process</p>
      </div>
      ...
      <div className="relative min-h-[700px] flex items-center justify-center">
        ...
        <div className="absolute right-12 top-1/2 transform -translate-y-1/2 z-30">
        ...
        <div className="absolute top-4 left-0 right-0 flex justify-around z-20">
          {categories.slice(0, 3).map((category, index) => {
            const config = CATEGORY_CONFIG[category.label as keyof typeof CATEGORY_CONFIG]
            const Icon = config?.icon || Factory
            const isSelected = selectedCategory === category.label
            const positions = ["left-[8%]", "left-[35%]", "left-[62%]"]

            return (
              <div key={category.label} className={`absolute ${positions[index]} w-72`}>
                {/* Connecting line */}
                <div
                ...
                {/* Category header */}
                <div
                  className={`bg-gradient-to-r ${config?.gradient} text-white px-6 py-4 rounded-2xl shadow-xl cursor-pointer transform transition-all duration-300 hover:scale-105 hover:shadow-2xl ${isSelected ? "ring-4 ring-yellow-400 scale-105" : ""} relative overflow-hidden group`}
                  onClick={() => onCategorySelect(category.label)}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent transform -skew-x-12 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
                  <div className="relative flex items-center justify-center gap-3">
                    <Icon className="w-6 h-6" />
                    <span className="font-bold text-lg">{category.label.toUpperCase()}</span>
                  </div>
                </div>
                ...
                >
                  <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                    {category.causes.slice(0, isSelected ? category.causes.length : 3).map((cause, i) => (
                      <div
                        key={i}
            ...
            return (
              <div key={category.label} className={`absolute ${positions[index]} w-72`}>
                ...
                <div
                  ...
                  style={{ background: `linear-gradient(to top, ${config?.accentColor}, transparent)` }}
                ></div>
              </div>
            )
          })}
...
const ROIImpactCard = ({ processData }: { processData: (typeof enhancedProcessData)[string] }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
      ...
      <Card className="bg-gradient-to-br from-green-50 via-emerald-50 to-green-100 dark:from-green-900/20 dark:to-emerald-900/20 border-green-200 dark:border-green-800 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <CardContent className="space-y-6">
          <div className="flex justify-between items-center p-3 bg-white/60 dark:bg-slate-800/60 rounded-xl">
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Without Zauvijek:</span>
            <Badge variant="destructive" className="text-sm px-3 py-1">
              {processData.roi.withoutZauvijek}
            </Badge>
          </div>
          <div className="flex justify-between items-center p-3 bg-white/60 dark:bg-slate-800/60 rounded-xl">
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">With Zauvijek:</span>
          ...
          <div className="flex justify-between items-center pt-4 border-t border-green-200 dark:border-green-700">
            <span className="font-bold text-green-700 dark:text-green-400">Total Saving:</span>
      ...
      <Card className="bg-gradient-to-br from-blue-50 via-cyan-50 to-blue-100 dark:from-blue-900/20 dark:to-cyan-900/20 border-blue-200 dark:border-blue-800 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <CardContent className="space-y-6">
          <div className="flex justify-between items-center p-3 bg-white/60 dark:bg-slate-800/60 rounded-xl">
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Before:</span>
            <Badge variant="destructive" className="text-sm px-3 py-1">
              {processData.roi.kwhBefore}
            </Badge>
          </div>
          <div className="flex justify-between items-center p-3 bg-white/60 dark:bg-slate-800/60 rounded-xl">
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">After:</span>
            <Badge variant="secondary" className="text-sm px-3 py-1">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-blue-200 dark:border-blue-700">
            <span className="font-bold text-blue-700 dark:text-blue-400">Reduction:</span>
      ...
      <Card className="bg-gradient-to-br from-purple-50 via-violet-50 to-purple-100 dark:from-purple-900/20 dark:to-violet-900/20 border-purple-200 dark:border-purple-800 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <CardContent className="space-y-6">
          <div className="p-4 bg-white/60 dark:bg-slate-800/60 rounded-xl">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{processData.technology}</p>
          </div>
          <div className="p-4 bg-white/60 dark:bg-slate-800/60 rounded-xl">
            <h4 className="font-bold text-sm text-purple-700 dark:text-purple-400 mb-3 flex items-center gap-2">
              <ChevronRight className="w-4 h-4" />
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{processData.impact}</p>
          </div>
        </CardContent>
      </Card>
    </div>
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-xl text-slate-600 dark:text-slate-400 max-w-3xl mx-auto leading-relaxed">
            Comprehensive root cause analysis with ROI impact assessment for foundry processes
          </p>
        </div>
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-3 text-2xl">
                <div className="p-2 bg-orange-100 dark:bg-orange-800/30 rounded-xl">
                  {React.createElement(
                    CATEGORY_CONFIG[selectedCategory as keyof typeof CATEGORY_CONFIG]?.icon || Factory,
              ...
              <div className="space-y-6">
                <div className="p-6 bg-gradient-to-r from-slate-50 to-blue-50 dark:from-slate-800 dark:to-slate-700 rounded-2xl">
                  <p className="text-lg text-slate-600 dark:text-slate-400 leading-relaxed">
                    {CATEGORY_CONFIG[selectedCategory as keyof typeof CATEGORY_CONFIG]?.description}
                  </p>
                </div>
                  ...
                  {currentProcessData.fishbone
                    ...
                    ?.causes.map((cause, index) => (
                      ...
                      >
                        ...
                        <span className="text-slate-700 dark:text-slate-300 leading-relaxed font-medium">{cause}</span>
                      </div>
                    ))}
                </div>
              </div>
